...
export const RulesSnapshotSchema = z.object({
  team_size: z.number().int().min(1).max(11).default(6),
  game_minutes: z.number().int().positive().default(8),
  extra_minutes: z.number().int().nonnegative().default(2),
  max_goals_to_win: z.number().int().positive().default(2),
  penalties_on_tie: z.boolean().default(true),
  penalty_win_weight: z.number().min(0).max(1).default(0.5),
  points: z.object({
    loss: z.number().default(0),
    draw: z.number().default(1),
    penalty_bonus_win: z.number().default(2),
    regulation_win: z.number().default(3),
  }).default({
    loss: 0,
    draw: 1,
    penalty_bonus_win: 2,
...
export const MatchdayCreateSchema = z.object({
  ...
  maxPlayers: z.number().int().min(6).max(30).default(18),
